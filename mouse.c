...
typedef struct
{
	Boolean_t Cuadrado	        : 1;
	Boolean_t AbrirPaint		: 1;
	Boolean_t Dibuja_7 			: 1;
	Boolean_t Notepad	        : 1;
	Boolean_t move_task			: 1;
	Boolean_t HolaMundo		    : 1;
	Boolean_t Control_C			: 1;
	Boolean_t Control_V		     : 1;
}Function_Flags;
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
static usb_status_t USB_DeviceHidMouseAction(void)
{
	...
	{
		case T1:
			/*Respuesta de la funcion de dibujar el cuadrado*/
			flags.Cuadrado	 = Cuadrado(g_UsbDeviceHidMouse.mouse_buffer);
			...
			return USB_DeviceHidSend(g_UsbDeviceHidMouse.hidHandle,USB_HID_MOUSE_ENDPOINT_IN,g_UsbDeviceHidMouse.mouse_buffer,USB_HID_MOUSE_REPORT_LENGTH);
		break;
		case T2:
			/*Respuesta de la funcion de abrir en la ventana de comando Paint*/
			flags.AbrirPaint = Paint(g_UsbDeviceHidMouse.keyboard_buffer);
			...
			return USB_DeviceHidSend(g_UsbDeviceHidMouse.hidHandle, USB_HID_MOUSE_ENDPOINT_IN,g_UsbDeviceHidMouse.keyboard_buffer, USB_HID_KEYBOARD_REPORT_LENGTH);
		break;
		case T3:
			/*Respuesta de la funcion de dibujar el numero del equipo*/
			flags.Dibuja_7 = draw(g_UsbDeviceHidMouse.mouse_buffer);
		...
		break;
		case T4:
			/*Respuesta de la funcion de abrir en la ventana de comando NotePad*/
			flags.Notepad = Notepad(g_UsbDeviceHidMouse.keyboard_buffer);
		...
		break;
		case T5:
			/*Respuesta de la funcion de mover el mouse*/
			flags.move_task = move(g_UsbDeviceHidMouse.mouse_buffer, LEFT);

			if(flags.move_task )
				{
					currentTask++;
					flags.move_task = FALSE; /*setting flag to false for a future move */
				}

			return USB_DeviceHidSend(g_UsbDeviceHidMouse.hidHandle,USB_HID_MOUSE_ENDPOINT_IN,g_UsbDeviceHidMouse.mouse_buffer,USB_HID_MOUSE_REPORT_LENGTH);
		break;
		case T6:
			/*Respuesta de la funcion de escribir el mensaje HolaMundo*/
			flags.HolaMundo = write(g_UsbDeviceHidMouse.keyboard_buffer);
		...
		break;
		case T7:
			/*Respuesta de la funcion de copiar con comandos el texto*/
			flags.Control_C = copy(g_UsbDeviceHidMouse.keyboard_buffer);
		...
		break;
		case T9:
			/*Respuesta de la funcion de pegar con comandos el texto*/
			flags.Control_C = paste(g_UsbDeviceHidMouse.keyboard_buffer);
